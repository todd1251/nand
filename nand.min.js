...
window.addEventListener("load",function(){var a=document.getElementById("paper");paper.setup(a);paper.view.onFrame=function(a){paper.view.draw()};paper.view.onResize=function(a){render()};render()});function OrView(a,b){var c=a.width/7,d=a.height/2,g=new paper.Path.Arc({from:{x:a.narrowSegmentWidth,y:0},through:{x:a.narrowSegmentWidth+a.wideSegmentWidth/2,y:d/4},to:{x:a.width,y:d}});g.remove();var e=new paper.Path.Arc({from:{x:a.width,y:d},through:{x:a.narrowSegmentWidth+a.wideSegmentWidth/2,y:a.height-d/4},to:{x:a.narrowSegmentWidth,y:a.height}});e.remove();d=new paper.Path.Arc({from:{x:0,y:a.height},through:{x:c,y:d},to:{x:0,y:0}});d.remove();g=new paper.Path({segments:[{x:0,y:0},{x:a.narrowSegmentWidth,
y:0},g.segments[0],g.segments[1],e.segments[0],e.segments[1],{x:a.narrowSegmentWidth,y:a.height},{x:0,y:a.height},d.segments[0],d.segments[1]],closed:!0,strokeWidth:b.stroke.width,strokeColor:b.stroke.colour,fillColor:b.fill.colour});d=a.height/4;e=new paper.Path.Line({from:{x:-(b.stroke.width/2),y:d},to:{x:c,y:d},strokeWidth:.6*b.stroke.width,strokeColor:b.stroke.colour});c=new paper.Path.Line({from:{x:-(b.stroke.width/2),y:a.height-d},to:{x:c,y:a.height-d},strokeWidth:.6*b.stroke.width,strokeColor:b.stroke.colour});
return new paper.Group([e,c,g])};function XorView(a,b){var c=new OrView(a,b),d=c.bounds.height,g=2.5*b.stroke.width,e=new paper.Path.Arc({from:{x:-g,y:0},through:{x:a.width/7-.85*g,y:d/2},to:{x:-g,y:d}});e.remove();g=(100<c.gateWidth?2:1)-g;d=new paper.Path({segments:[{x:g,y:0},e.segments[0],e.segments[1],{x:g,y:d}],strokeWidth:b.stroke.width,strokeColor:b.stroke.colour});return new paper.Group([c,d])}
...
{x:1-e,y:b}],strokeWidth:c.stroke.width,strokeColor:c.stroke.colour,strokeCap:"square"});return new paper.Group([a,c,d])};
