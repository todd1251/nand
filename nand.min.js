...
function redrawWires(a){wires.map(function(c){var b=c.from,d=c.to,f=c.input;null!=c.path&&(null!=c.path.crossings&&c.path.crossings.map(function(a){a.remove()}),c.path.remove(),c.path=null);var e=a.createPath();e.strokeWidth=a.wireStrokeWidth;e.fillColor=null;b={x:b.position.x-b.bounds.width/2+b.outputs[0].x-b.inputs[0].x,y:b.position.y-b.bounds.height/2+b.outputs[0].y};d={x:d.position.x-d.bounds.width/2,y:d.position.y-d.bounds.height/2+d.inputs[f].y};f=d.x-b.x;e.moveTo({x:b.x,y:b.y});e.lineTo({x:b.x+
f/2,y:b.y});e.lineTo({x:b.x+f/2,y:d.y});e.lineTo({x:d.x,y:d.y});e.crossings=[];wires.map(function(b){if(null!=b.path)if(b=b.path.getIntersections(e),0<b.length){console.log(b.length+" intersection(s)");for(var c=0;c<b.length;c++){var d=b[c];e.crossings.push(new paper.Path.Circle({radius:5,center:[d.point.x,d.point.y],strokeWidth:a.wireStrokeWidth,strokeColor:a.stroke.colour}))}}else console.log("no intersections")});c.path=e})}function MetaDragToConnect(a,c){this.element=a;this.appearance=c}
