...
DragToMove.prototype.dragFinished=function(a){this.dragPosition=null;redrawWires(this.appearance)};function dragToPan(){function a(a){c=null}var c;paper.view.onMouseDown=function(a){c=a.point};paper.view.onMouseDrag=function(a){a=a.point;paper.view.center=new paper.Point({x:paper.view.center.x-(a.x-c.x),y:paper.view.center.y-(a.y-c.y)})};paper.view.onMouseUp=a;paper.view.onMouseOut=a};function FanOutView(a,c,b){a=new paper.Path.Circle({radius:2,center:[0,0],strokeWidth:b.wireStrokeWidth,strokeColor:b.stroke.colour,fillColor:b.stroke.colour});a.alignmentCentre={x:0,y:0};a.inputs=[{x:0,y:0}];a.outputs=[{x:0,y:-5},{x:0,y:5}];return a};function GateView(a,c,b){a=new a(c,b);var d=c.width/2-a.alignmentCentre.x+a.inputs[0].x/2,f=[];f.push(a);c=[{x:c.width-d+a.inputs[0].x,y:a.outputs[0].y}];for(var e=new paper.Path.Line({from:a.outputs[0],to:c[0],strokeWidth:b.wireStrokeWidth,strokeColor:b.stroke.colour}),g=[],k=0;k<a.inputs.length;k++){var h=a.inputs[k];g[k]={x:h.x-d,y:h.y};f.push(new paper.Path.Line({from:g[k],to:{x:h.x,y:h.y},strokeWidth:b.wireStrokeWidth,strokeColor:b.stroke.colour}))}f.push(e);a=new paper.Group(f);a.inputs=g;a.outputs=
c;return a};function clamp(a,c,b){return Math.min(Math.max(a,c),b)};var wires=[];
function redrawWires(a){wires.map(function(c){var b=c.from,d=c.to,e=c.input;null!=c.path&&(null!=c.path.markers&&c.path.markers.map(function(a){a.remove()}),c.path.remove(),c.path=null);var f=a.createPath();f.strokeWidth=a.wireStrokeWidth;f.fillColor=null;b={x:b.position.x-b.bounds.width/2+b.outputs[0].x-b.inputs[0].x,y:b.position.y-b.bounds.height/2+b.outputs[0].y};d={x:d.position.x-d.bounds.width/2,y:d.position.y-d.bounds.height/2+d.inputs[e].y};e=d.x-b.x;f.moveTo({x:b.x,y:b.y});f.lineTo({x:b.x+e/
2,y:b.y});f.lineTo({x:b.x+e/2,y:d.y});f.lineTo({x:d.x,y:d.y});f.markers=[];c.path=f;c.pointFrom=b;c.pointTo=d});for(var c=[],b=0;b<wires.length;b++)for(var d=wires[b].path,f=b+1;f<wires.length;f++){var e=wires[f];if(null!=e.path){for(var g=[],k=e.path.getIntersections(d),h=0;h<k.length;h++)g.push(k[h].point);k=c.length;for(h=0;h<c.length;h++)if(c[h].wire==e){for(var k=h,n=0;n<c[h].points;n++)g.push(c[h].points[n]);break}g.sort(function(a,b){return a.x-b.x});c[k]={wire:e,intersections:g}}}for(f=0;f<
c.length;f++)if(b=c[f],k=b.intersections,d=b.wire.path,e=b.wire.pointFrom,g=b.wire.pointTo,0<k.length){n=d.markers;d.remove();d=a.createPath();d.strokeWidth=a.wireStrokeWidth;d.fillColor=null;var m=g.x-e.x;d.moveTo({x:e.x,y:e.y});for(h=0;h<k.length;h++){var l=k[h];console.log("#"+h+" at ("+l.x+", "+l.y+")")}h=0;l=k[h];l.x>=e.x&&l.x<=e.x+m/2&&l.y==e.y&&(console.log("intersection #"+f+" on first leg"),d.lineTo({x:l.x-5,y:e.y}),d.arcTo({x:l.x,y:e.y-5},{x:l.x+5,y:e.y}),h++);d.lineTo({x:e.x+m/2,y:e.y});
...
new c));a(AndView.bind(null,new b));var e=a(AndView.bind(null,new c));a(OrView.bind(null,new XorDecoration,new b));var g=a(FanOutView.bind(null,new b));new paper.Path.Rectangle({point:[0,0],size:[paper.view.bounds.width,paper.view.bounds.height],strokeColor:"#ccc"});var k=50;[a(BufferView.bind(null,new b)),a(BufferView.bind(null,new c)),a(AndView.bind(null,new b)),a(AndView.bind(null,new c)),a(OrView.bind(null,new b,new b)),a(OrView.bind(null,new b,new c)),a(OrView.bind(null,new XorDecoration,new b)),
a(OrView.bind(null,new XorDecoration,new c))].map(function(a){var b=new DimensionsProportionalToWidth(50),c=new AppearanceProportionalToDimensions(b),d=new AppearanceProportionalToDimensions(b);d.stroke.colour="#ccc";d=new a(b,d);d.position={x:k,y:50};k+=1.1*d.bounds.width;dragToCopy(d,a,b,c)});var h=new DimensionsProportionalToWidth(50),n=new AppearanceProportionalToDimensions(h),m=d({x:150,y:150},f),g=d({x:200,y:150},g),f=d({x:150,y:250},f),l=d({x:250,y:200},e),p=d({x:350,y:150},e),q=d({x:350,y:250},
e),e=d({x:450,y:200},e);wires.push({from:m,to:g,input:0});wires.push({from:g,to:p,input:0});wires.push({from:m,to:l,input:0});wires.push({from:f,to:l,input:1});wires.push({from:f,to:q,input:1});wires.push({from:l,to:p,input:1});wires.push({from:l,to:q,input:0});wires.push({from:p,to:e,input:0});wires.push({from:q,to:e,input:1});redrawWires(n)}
