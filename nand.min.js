...
2,y:c.y});e.lineTo({x:c.x+f/2,y:d.y});e.lineTo({x:d.x,y:d.y});e.markers=[];b.path=e;b.pointFrom=c;b.pointTo=d});for(var c=[],b=0;b<wires.length;b++)for(var d=wires[b],f=d.path,e=b+1;e<wires.length;e++){var g=wires[e];if(null!=g.path){for(var h=[],k=g.path.getIntersections(f),l=0;l<k.length;l++)h.push(k[l].point);c.push({first:d,second:g,intersections:h})}}console.log(c);for(e=0;e<c.length;e++){b=c[e];k=b.intersections;f=b.second.path;d=f.markers;for(l=0;l<k.length;l++)g=k[l],console.log("#"+l+" at ("+
g.x+", "+g.y+")"),g=new paper.Path.Circle({radius:3,center:[g.x,g.y],strokeWidth:a.wireStrokeWidth,strokeColor:a.stroke.colour}),d.push(g);b.second.path=f}}function MetaDragToConnect(a,c){this.element=a;this.appearance=c}MetaDragToConnect.prototype.dragStarted=function(a){this.originalPosition=this.element.position;this.dragPosition=a.point;a.event.metaKey?(this.connecting=!0,a.stopPropagation()):this.connecting=!1};
...
c=new AppearanceProportionalToDimensions(b),d=new AppearanceProportionalToDimensions(b);d.stroke.colour="#ccc";d=new a(b,d);d.position={x:g,y:50};g+=1.1*d.bounds.width;dragToCopy(d,a,b,c)});var h=new DimensionsProportionalToWidth(50),k=new AppearanceProportionalToDimensions(h),l=d({x:150,y:150},f),f=d({x:150,y:250},f),e=d({x:250,y:200},e);wires.push({from:l,to:e,input:0});wires.push({from:f,to:e,input:1});redrawWires(k)}
