...
e/2,y:c.y});g.lineTo({x:c.x+e/2,y:d.y});g.lineTo({x:d.x,y:d.y});g.lineTo({x:d.x+1,y:d.y});g.markers=[];b.path=g;b.pointFrom=c;b.pointTo=d});for(var c=[],b=0;b<wires.length;b++)for(var e=wires[b].path,f=b+1;f<wires.length;f++){var d=wires[f];if(null!=d.path){for(var g=[],h=d.path.getIntersections(e),k=0;k<h.length;k++)g.push(h[k].point);h=c.length;for(k=0;k<c.length;k++)if(c[k].wire==d){for(var h=k,p=0;p<c[k].points;p++)g.push(c[k].points[p]);break}g.sort(function(a,b){return a.x-b.x});c[h]={wire:d,
intersections:g}}}for(f=0;f<c.length;f++)if(b=c[f],h=b.intersections,e=b.wire.path,d=b.wire.pointFrom,g=b.wire.pointTo,0<h.length){p=e.markers;e.remove();e=a.createPath();e.strokeWidth=a.wireStrokeWidth;e.fillColor=null;var m=g.x-d.x;e.moveTo({x:d.x,y:d.y});for(k=0;k<h.length;k++)var l=h[k];k=0;l=h[k];l.x>=d.x&&l.x<=d.x+m/2&&l.y==d.y&&(e.lineTo({x:l.x-5,y:d.y}),e.arcTo({x:l.x,y:d.y-5},{x:l.x+5,y:d.y}),k++);e.lineTo({x:d.x+m/2,y:d.y});k<h.length&&(l=h[k],l.x==d.x+m/2&&l.y>=d.y&&l.y<=g.y&&(e.lineTo({x:d.x+
m/2,y:l.y-5}),e.arcTo({x:d.x+m/2+5,y:l.y},{x:d.x+m/2,y:l.y+5}),k++));k<h.length&&(l=h[k],l.x==d.x+m/2&&l.y>=g.y&&l.y<=d.y&&(e.lineTo({x:d.x+m/2,y:l.y+5}),e.arcTo({x:d.x+m/2+5,y:l.y},{x:d.x+m/2,y:l.y-5}),k++));e.lineTo({x:d.x+m/2,y:g.y});k<h.length&&(l=h[k],l.x>=d.x+m/2&&l.x<=g.x&&l.y==g.y&&(e.lineTo({x:l.x-5,y:g.y}),e.arcTo({x:l.x,y:g.y-5},{x:l.x+5,y:g.y})));e.lineTo({x:g.x,y:g.y});e.markers=p;b.wire.path=e}}function MetaDragToConnect(a,c,b){this.element=a;this.dimensions=c;this.appearance=b}
...
MetaDragToConnect.prototype.drag=function(a){if(this.connecting){null!=this.path&&this.path.remove();this.path=this.appearance.createPath();this.path.strokeWidth=this.appearance.wireStrokeWidth;this.path.fillColor=null;var c=this.element.output;null==c?c=0:c++;var b=this.element.position.x-this.element.bounds.width/2+this.element.outputs[c].x-this.element.inputs[0].x,c=this.element.position.y-this.element.bounds.height/2+this.element.outputs[c].y,e=a.point.x,f=a.point.y,d=e-b;this.path.moveTo({x:b,
y:c});this.path.lineTo({x:b+d/2,y:c});this.path.lineTo({x:b+d/2,y:f});this.path.lineTo({x:e,y:f});a.stopPropagation()}};MetaDragToConnect.prototype.dragFinished=function(a){this.dragPosition=null;null!=this.path&&this.path.remove();if(this.connecting){var c=paper.project.hitTestAll(a.point);if(0<c.length){for(var b=null,e=0;e<c.length;e++)if(null!=c[e].item.inputs){b=c[e];break}if(null==b){console.log("no hit");return}connect(this.element,b.item,this.dimensions,this.appearance)}a.stopPropagation()}};
...
b,e),new DragToMove(a,e)])}wires.push({from:a,to:c,input:f,output:d});redrawWires(e);return a}}};function render(){function a(a,b,c){b=b||new NoDecoration;c=c||new NoDecoration;return GateView.bind(null,a.bind(null,b,c))}function c(a,b){var c=b(l,n);c.position=a;dragHandlerChain(c,[new MetaDragToConnect(c,l,n),new DragToMove(c,n)]);return c}paper.project.activeLayer.removeChildren();wires=[];new paper.Path.Rectangle({point:[0,0],size:[paper.view.bounds.width,paper.view.bounds.height],strokeColor:"#ccc"});var b=a(BufferView),e=a(BufferView,void 0,new InverterDecoration),f=a(AndView),d=a(AndView,
void 0,new InverterDecoration),g=a(OrView),h=a(OrView,void 0,new InverterDecoration),k=a(XorView),p=a(XorView,void 0,new InverterDecoration),m=50;[b,e,f,d,g,h,k,p].map(function(a){var b=new DimensionsProportionalToWidth(50),c=new AppearanceProportionalToDimensions(b),d=new AppearanceProportionalToDimensions(b);d.stroke.colour="#ccc";d=new a(b,d);d.position={x:m,y:50};m+=1.1*d.bounds.width;dragToCopy(d,a,b,c)});var l=new DimensionsProportionalToWidth(50),n=new AppearanceProportionalToDimensions(l),
d=c({x:120,y:150},b),g=c({x:150,y:200},b),h=c({x:250,y:146},k),p=c({x:250,y:204},f),d=connect(d,h,l,n),d=connect(d,p,l,n),g=connect(g,h,l,n),g=connect(g,p,l,n),e=c({x:420,y:150},b),b=c({x:450,y:200},b),k=c({x:550,y:146},k),f=c({x:550,y:204},f),e=connect(e,k,l,n),e=connect(e,f,l,n),b=connect(b,k,l,n),b=connect(b,f,l,n),d=new CircuitView([d,g,h,p],l,n),f=new CircuitView([e,b,k,f],l,n);connect(d,f,l,n)}
