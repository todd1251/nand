...
window.addEventListener("load",function(){var a=document.getElementById("paper");paper.setup(a);paper.view.onFrame=function(a){paper.view.draw()};paper.view.onResize=function(a){render()};render()});function OrView(a,b){var c=a.width/7,d=a.height/2,g=b.createPath();g.moveTo({x:b.stroke.width/2,y:0});g.lineTo({x:a.narrowSegmentWidth,y:0});g.arcTo({x:a.narrowSegmentWidth+a.wideSegmentWidth/2,y:d/4},{x:a.width,y:d});g.arcTo({x:a.narrowSegmentWidth+a.wideSegmentWidth/2,y:a.height-d/4},{x:a.narrowSegmentWidth,y:a.height});g.lineTo({x:b.stroke.width/2,y:a.height});g.arcTo({x:c,y:d},{x:b.stroke.width/2,y:0});g.closePath();var e=a.height/4,d=new paper.Path.Line({from:{x:-(b.stroke.width/2),y:e},to:{x:c,
y:e},strokeWidth:.6*b.stroke.width,strokeColor:b.stroke.colour}),c=new paper.Path.Line({from:{x:-(b.stroke.width/2),y:a.height-e},to:{x:c,y:a.height-e},strokeWidth:.6*b.stroke.width,strokeColor:b.stroke.colour});return new paper.Group([d,c,g])};function XorViewDecorator(a,b,c){a=new a(b,c);var d=b.width/7;b=a.bounds.height;var g=b/2,e=2.5*c.stroke.width,h=new paper.Path.Arc({from:{x:-e,y:0},through:{x:d-.85*e,y:g},to:{x:-e,y:b}});h.remove();d=new paper.Path.Arc({from:{x:c.stroke.width-1-e,y:-c.stroke.width/2},through:{x:d-.85*e+c.stroke.width,y:g},to:{x:c.stroke.width-1-e,y:b+c.stroke.width/2},strokeWidth:c.stroke.width,strokeColor:"white",strokeCap:"square"});d.remove();c=new paper.Path({segments:[{x:1-e,y:0},h.segments[0],h.segments[1],
