...
return b};function InverterView(a){a=a||{};return new paper.Path.Circle({radius:a.radius||12.5,strokeWidth:a.strokeWidth||4,strokeColor:a.strokeColor||"black",fillColor:a.fillColor||"white"})};function render(){paper.project.activeLayer.removeChildren();(new BufferView).position={x:150,y:100};(new BufferView({width:150})).position={x:450,y:100};(new NotView).position={x:150,y:300};(new NotView({width:150})).position={x:450,y:300};(new AndView).position={x:750,y:100};(new AndView({width:150})).position={x:1050,y:100};(new NandView).position={x:750,y:300};(new NandView({width:150})).position={x:1050,y:300};(new OrView).position={x:150,y:500};(new OrView({width:150})).position={x:450,y:500};
(new NorView).position={x:150,y:700};(new NorView({width:150})).position={x:450,y:700};(new XorView).position={x:750,y:500};(new XorView({width:150})).position={x:1050,y:500};(new XNorView).position={x:750,y:700};(new XNorView({width:150})).position={x:1050,y:700}}
window.addEventListener("load",function(){paper.setup(document.getElementById("paper"));paper.view.onFrame=function(a){paper.view.draw()};paper.view.onResize=function(a){render()};var a;paper.view.onMouseDown=function(b){(b=paper.project.hitTest(b.point,{handles:!0,tolerance:5}))&&(a="handle-in"==b.type?b.segment.handleIn:b.segment.handleOut)};(new paper.Tool).onMouseDrag=function(b){a&&(a.x+=b.delta.x,a.y+=b.delta.y,console.log("x: "+a.x+", y: "+a.y))};render()});function NandView(a){a=a||{};var b=new AndView(a),c=b.gateWidth/6;a.radius=c/2;a=new InverterView(a);a.position={x:b.gateWidth+c/2+1,y:b.bounds.height/2};return new paper.Group([b,a])};function NorView(a){a=a||{};var b=new OrView(a),c=b.gateWidth/6;a.radius=c/2;a=new InverterView(a);a.position={x:b.gateWidth+c/2+1,y:b.bounds.height/2};return new paper.Group([b,a])};function NotView(a){a=a||{};var b=new BufferView(a),c=5*b.gateWidth/3/6;a.radius=c/2;a=new InverterView(a);a.position={x:b.gateWidth+c/2+1,y:b.bounds.height/2};return new paper.Group([b,a])};function OrView(a){a=a||{};var b=a.strokeWidth||4,c=a.strokeColor||"black",e=a.width||100,d=.4*e,f=.6*e,h=e/7,g=4*e/5,k=g/2,l=new paper.Path.Arc({from:{x:d,y:0},through:{x:d+f/2,y:k/4},to:{x:e,y:k}});l.remove();f=new paper.Path.Arc({from:{x:e,y:k},through:{x:d+f/2,y:g-k/4},to:{x:d,y:g}});f.remove();var m=new paper.Path.Arc({from:{x:0,y:g},through:{x:h,y:k},to:{x:0,y:0}});m.remove();a=new paper.Path({segments:[{x:0,y:0},{x:d,y:0},l.segments[0],l.segments[1],f.segments[0],f.segments[1],{x:d,y:g},{x:0,
...
return b};function XNorView(a){a=a||{};var b=new XorView(a),c=b.bounds.width/6;a.radius=c/2;a=new InverterView(a);a.position={x:b.bounds.width-c/2+c/2+1,y:b.bounds.height/2};return new paper.Group([b,a])};function XorView(a){a=a||{};var b=new OrView(a),c=b.bounds.width/7,e=.6*c,d=b.bounds.height,c=new paper.Path.Arc({from:{x:-e,y:0},through:{x:c-.85*e,y:d/2},to:{x:-e,y:d}});c.remove();a=new paper.Path({segments:[{x:1-e,y:0},c.segments[0],c.segments[1],{x:1-e,y:d}],strokeWidth:a.strokeWidth||4,strokeColor:a.strokeColor||"black",fillColor:a.fillColor||"white"});return new paper.Group([b,a])};
