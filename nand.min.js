...
DragToMove.prototype.dragFinished=function(a){this.dragPosition=null;redrawWires(this.appearance)};function dragToPan(){function a(a){c=null}var c;paper.view.onMouseDown=function(a){c=a.point};paper.view.onMouseDrag=function(a){a=a.point;paper.view.center=new paper.Point({x:paper.view.center.x-(a.x-c.x),y:paper.view.center.y-(a.y-c.y)})};paper.view.onMouseUp=a;paper.view.onMouseOut=a};function FanOutView(a,c,b){a=new a(c,b);c=new paper.Path.Circle({radius:2,center:[a.outputs[0].x,a.outputs[0].y],strokeWidth:b.wireStrokeWidth,strokeColor:b.stroke.colour,fillColor:b.stroke.colour});var d=[];d.push(a);d.push(c);d.push(new paper.Path.Line({from:a.outputs[0],to:{x:a.outputs[0].x,y:a.outputs[0].y-10},strokeWidth:b.wireStrokeWidth,strokeColor:b.stroke.colour}));d.push(new paper.Path.Line({from:a.outputs[0],to:{x:a.outputs[0].x,y:a.outputs[0].y+10},strokeWidth:b.wireStrokeWidth,strokeColor:b.stroke.colour}));
b=new paper.Group(d);b.alignmentCentre={x:a.bounds.width/2,y:a.bounds.height/2};b.inputs=a.inputs;b.outputs=[{x:a.outputs[0].x,y:a.outputs[0].y-10},{x:a.outputs[0].x,y:a.outputs[0].y+10}];return b};function GateView(a,c,b){a=new a(c,b);var d=c.width/2-a.alignmentCentre.x+a.inputs[0].x/2,f=[];f.push(a);c=[{x:c.width-d+a.inputs[0].x,y:a.outputs[0].y}];for(var e=new paper.Path.Line({from:a.outputs[0],to:c[0],strokeWidth:b.wireStrokeWidth,strokeColor:b.stroke.colour}),g=[],k=0;k<a.inputs.length;k++){var h=a.inputs[k];g[k]={x:h.x-d,y:h.y};f.push(new paper.Path.Line({from:g[k],to:{x:h.x,y:h.y},strokeWidth:b.wireStrokeWidth,strokeColor:b.stroke.colour}))}f.push(e);b=new paper.Group(f);b.alignmentCentre=
{x:a.bounds.width/2,y:a.bounds.height/2};b.inputs=g;b.outputs=c;return b};function clamp(a,c,b){return Math.min(Math.max(a,c),b)};var wires=[];
...
new c))].map(function(a){var b=new DimensionsProportionalToWidth(50),c=new AppearanceProportionalToDimensions(b),d=new AppearanceProportionalToDimensions(b);d.stroke.colour="#ccc";d=new a(b,d);d.position={x:g,y:50};g+=1.1*d.bounds.width;dragToCopy(d,a,b,c)});var k=new DimensionsProportionalToWidth(50),h=new AppearanceProportionalToDimensions(k),n=d({x:150,y:150},FanOutView.bind(null,f)),f=d({x:150,y:250},FanOutView.bind(null,f)),m=d({x:250,y:200},FanOutView.bind(null,e)),l=d({x:350,y:150},e),p=d({x:350,
y:250},e),e=d({x:450,y:200},e);wires.push({from:n,output:0,to:l,input:0});wires.push({from:n,output:1,to:m,input:0});wires.push({from:f,output:0,to:m,input:1});wires.push({from:f,output:1,to:p,input:1});wires.push({from:m,output:0,to:l,input:1});wires.push({from:m,output:1,to:p,input:0});wires.push({from:l,output:0,to:e,input:0});wires.push({from:p,output:0,to:e,input:1});redrawWires(h)}
