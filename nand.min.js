...
e/2,y:c.y});g.lineTo({x:c.x+e/2,y:d.y});g.lineTo({x:d.x,y:d.y});g.markers=[];b.path=g;b.pointFrom=c;b.pointTo=d});for(var c=[],b=0;b<wires.length;b++)for(var d=wires[b].path,f=b+1;f<wires.length;f++){var e=wires[f];if(null!=e.path){for(var g=[],k=e.path.getIntersections(d),h=0;h<k.length;h++)g.push(k[h].point);k=c.length;for(h=0;h<c.length;h++)if(c[h].wire==e){for(var k=h,n=0;n<c[h].points;n++)g.push(c[h].points[n]);break}g.sort(function(a,b){return a.x-b.x});c[k]={wire:e,intersections:g}}}for(f=
...
" on third leg"),d.lineTo({x:l.x-5,y:g.y}),d.arcTo({x:l.x,y:g.y-5},{x:l.x+5,y:g.y})));d.lineTo({x:g.x,y:g.y});d.markers=n;b.wire.path=d}}function MetaDragToConnect(a,c){this.element=a;this.appearance=c}MetaDragToConnect.prototype.dragStarted=function(a){this.originalPosition=this.element.position;this.dragPosition=a.point;a.event.metaKey?(this.connecting=!0,a.stopPropagation()):this.connecting=!1};
...
MetaDragToConnect.prototype.dragFinished=function(a){this.dragPosition=null;null!=this.path&&this.path.remove();if(this.connecting){var c=paper.project.hitTestAll(a.point);console.log(c.length);if(0<c.length){for(var b=null,d=0;d<c.length;d++)if(null!=c[d].item.inputs){b=c[d];break}if(null==b){console.log("no hit");return}console.log(b.item.inputs);c=b.item.input;null==c?c=0:c++;if(c>=b.item.inputs.length)return;b.item.input=c;wires.push({from:this.element,to:b.item,input:c})}redrawWires(this.appearance);
a.stopPropagation()}};function render(){function a(a){return GateView.bind(null,a)}function c(){}function b(){}function d(a,b){var c=b(h,n);c.position=a;dragHandlerChain(c,[new MetaDragToConnect(c,n),new DragToMove(c,n)]);return c}paper.project.activeLayer.removeChildren();c.prototype.decorate=function(a,b,c){b=2*b.inverterRadius;a.arcBy({x:b,y:0},!1);a.arcBy({x:-b,y:0},!0);a.arcBy({x:b,y:0},!0);a.arcBy({x:-b,y:0},!1);return a};b.prototype.decorate=function(a,b,c){return a};var f=a(BufferView.bind(null,new b)),e=a(BufferView.bind(null,
new c));a(AndView.bind(null,new b));var g=a(AndView.bind(null,new c));a(OrView.bind(null,new XorDecoration,new b));new paper.Path.Rectangle({point:[0,0],size:[paper.view.bounds.width,paper.view.bounds.height],strokeColor:"#ccc"});var k=50;[a(BufferView.bind(null,new b)),a(BufferView.bind(null,new c)),a(AndView.bind(null,new b)),a(AndView.bind(null,new c)),a(OrView.bind(null,new b,new b)),a(OrView.bind(null,new b,new c)),a(OrView.bind(null,new XorDecoration,new b)),a(OrView.bind(null,new XorDecoration,
new c))].map(function(a){var b=new DimensionsProportionalToWidth(50),c=new AppearanceProportionalToDimensions(b),d=new AppearanceProportionalToDimensions(b);d.stroke.colour="#ccc";d=new a(b,d);d.position={x:k,y:50};k+=1.1*d.bounds.width;dragToCopy(d,a,b,c)});var h=new DimensionsProportionalToWidth(50),n=new AppearanceProportionalToDimensions(h),m=d({x:247,y:154},FanOutView.bind(null,f)),f=d({x:247,y:246},FanOutView.bind(null,f)),l=d({x:299,y:200},FanOutView.bind(null,g)),p=d({x:350,y:150},g),q=d({x:350,
y:250},g),g=d({x:401,y:200},g),e=d({x:450,y:200},e);wires.push({from:m,output:0,to:p,input:0});wires.push({from:m,output:1,to:l,input:0});wires.push({from:f,output:0,to:l,input:1});wires.push({from:f,output:1,to:q,input:1});wires.push({from:l,output:0,to:p,input:1});wires.push({from:l,output:1,to:q,input:0});wires.push({from:p,output:0,to:g,input:0});wires.push({from:q,output:0,to:g,input:1});wires.push({from:g,output:0,to:e,input:0});redrawWires(n)}
